...
bool flagCtemperatures, flagHPaBmp, flagEnvoie, flagEnvoieAuto, flagHdht, flagMontreheureRtc, flagEcritSd, flagGy49, flagVin, flagMesures;
...
//Mode deep sleep
unsigned long timeSleep = 900; //--> Durée du Deep Sleep (sec)
...
void loop() {
  ...
  if(cur_millis - pre_millis >= trigger_NSD){
    ...
    if(NbrMessagesEnvoyes == 2){ //--> Condition que le trigger_NSD est respecter en rapport avec la différence des millis actuel et de l'ancienne boucle.
